...
if [[ ! "$tun_mtu" =~ ^[0-9]+$ || "$tun_mtu" -lt 576 ]]; then
...
podman run -d --name "$peer_b_name" --network "$network" --ip 10.253.0.4 \
    ...
    sh -c '\
        ...
        exec /usr/local/bin/vela-cli peer up --state /bench/b --tun vela0 --mtu "$VELA_TUN_MTU" --bind 0.0.0.0:7001 --port 41000' \
    >/dev/null
...
podman run -d --name "$peer_a_name" --network "$network" --ip 10.253.0.3 \
    ...
    sh -c '\
        ...
        exec /usr/local/bin/vela-cli peer up --state /bench/a --tun vela0 --mtu "$VELA_TUN_MTU" --bind 0.0.0.0:7001 --port 41000' \
    >/dev/null
